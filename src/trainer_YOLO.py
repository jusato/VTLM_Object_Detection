...
class Trainer(object):
    ...
    def mask_out_image(self, img_boxes, img_feats, img_labels):
        ...
        for i in range(len(path_to_images)):
            command = './darknet detector test cfg/openimages.data cfg/yolov3-openimages.cfg /experiments/jsato/VTLM_object_detection/data/pretrained_models/yolov3-openimages.weights ' + path_to_images[i]
            command2 = 'cp predictions.jpg /data/jsato/object_detection_masking/' + images_ids[i] + '.jpg'
            os.system(command)
            os.system(command2)
